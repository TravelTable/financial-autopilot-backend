...
def list_subscriptions(user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
    subs = db.execute(select(Subscription).where(Subscription.user_id == user_id).order_by(Subscription.next_renewal_date.asc().nullslast(), Subscription.id.desc())).scalars().all()
    return [
        SubscriptionOut(
            id=s.id,
...
def ignore_subscription(subscription_id: int, user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
